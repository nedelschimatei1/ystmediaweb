    ...
    <section ref={sectionRef} className="w-full min-h-[50vh] md:h-full flex items-center justify-center bg-primary dark:bg-gradient-to-b dark:from-background dark:via-accent/5 dark:to-background text-primary-foreground dark:text-foreground overflow-hidden relative py-10 sm:py-12 md:py-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-3xl mx-auto text-center">
          {/* Badge */}
          <div 
            className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary-foreground/10 dark:bg-accent/10 border border-primary-foreground/20 dark:border-accent/20 text-primary-foreground dark:text-foreground text-xs font-medium mb-3 sm:mb-6 transition-all duration-700 ${
          ...
          {/* Headline */}
          <h2 
            className={`font-serif text-xl sm:text-3xl md:text-4xl lg:text-5xl font-medium tracking-tight leading-[1.1] text-primary-foreground dark:text-foreground transition-all duration-700 delay-100 ${
              isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
            }`}
          >
            {t("cta.title")}
          ...
          {/* Subtitle */}
          <p 
            className={`mt-2 sm:mt-4 text-sm text-primary-foreground/80 dark:text-foreground/80 leading-relaxed transition-all duration-700 delay-200 ${
              isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
            }`}
          >
            {t("cta.desc")}
          ...
          >
            <Link
              href="/contact#form"
              className="group w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 sm:px-7 py-2.5 sm:py-3.5 text-sm font-medium bg-primary-foreground text-primary dark:bg-foreground dark:text-background rounded-full transition-all duration-300 hover:scale-105 hover:shadow-lg dark:shadow-foreground/20 dark:hover:shadow-foreground/40 active:scale-100"
            >
              {t("cta.button")}
            ...
            <div className="flex flex-col items-center sm:items-start sm:border-l sm:border-primary-foreground/20 dark:sm:border-accent/30 sm:pl-6">
              <a
                ...
                className="text-base sm:text-lg font-semibold hover:underline transition-all dark:text-foreground"
              >
                +40 721 469 039
              </a>
              <span className="text-[10px] sm:text-xs text-primary-foreground/60 dark:text-foreground/60">{t("cta.hours")}</span>
            </div>
          </div>
        </div>
      </div>
